...
const cardStyle = 
{
    width: 'clamp(2rem, 80vw, 30rem)',
    height:'clamp(6rem, 60vh, 34rem)',
    maxHeight:isMobile?'44vh' : '34rem', 
    overflow: 'hidden',
    display: 'flex',
...
const Card = () =>
{
    ...
    return (
        <div style={cardStyle}>
            ...
            onHoverInChangeToColor = {()=>
            {
                dispatch(setBackColor(LIGHTMODE_BACKCOLOR))
            }}
            onHoverOutChangeToColor = {()=>
            {
                dispatch(setBackColor(STANDARDLIGHT_BACKCOLOR))
            }}
            />           
            <HalfCard 
            ThemeColor={'dark'}
            ...
            onHoverInChangeToColor={()=>
            {
                dispatch(setBackColor(DARKMODE_BACKCOLOR))
            }}
            onHoverOutChangeToColor = {()=>
            {
                dispatch(setBackColor(STANDARDLIGHT_BACKCOLOR))
